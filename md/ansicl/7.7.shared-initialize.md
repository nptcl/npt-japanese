...
そして`update-instance-for-different-class`によって呼び出されます。
...
あるいは、*slot-names*にシンボルである[`t`](4.4.t.html)を指定することもできます。
...
最適化では許可されています。
（この最適化は、`allocate-instance`メソッドに
